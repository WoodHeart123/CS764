...
public:
    ...
    std::vector<std::unique_ptr<Page>> readPagesFromDisk(unsigned long long offset, size_t recordSize, size_t numPages);
    //if offset is larger than sdd file size, write to the hdd file
    bool writePagesToDisk(unsigned long long offset, std::vector<std::unique_ptr<Page>> pages);
